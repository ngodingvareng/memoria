...
This document standardizes how code is written in `api/` — architecture,
file placement, naming, function signatures, error handling, code
ordering, and style. It extends the architecture summary in the root
`CLAUDE.md` with concrete, enforceable rules and templates. Every rule
below is grounded in a pattern already established in this codebase, or
in an issue found and fixed during a code review pass (referenced inline
where relevant) — this isn't a generic Go style guide, it's specific to
...
- `repository` implements interfaces **declared by `usecase`**, and is the
  only layer allowed to import `internal/db` (sqlc-generated). `pgtype`,
  `pgx`, and raw SQL awareness stop here — they never leak into `entity`
  or `usecase` as parameter/return types.
- `delivery/rest` (handler/dto/middleware) depends on `usecase` only,
...
- `internal/usecase/mocks` is 100% generated by Mockery. Never hand-edit
  — run `make mock-generate` after changing any interface listed in
  `.mockery.yaml` (add new interfaces there first).
...
- **One file per domain per concern.** Don't merge a mapper into its
  repository file, and don't split one domain's repository across two
  files. This is deliberate even when a file ends up small (mapper files
  in this codebase run 19–46 lines) — see §6 for why file-size alone
  isn't a reason to merge.
- **Cross-domain helpers get their own non-domain-prefixed file in the
  same package**, not duplicated per domain. Example:
...
- More than 2–3 domain parameters on a usecase method → wrap them in an
  ...
  here.")
- Repository method signatures use plain Go types and `uuid.UUID` /
  `*entity.X`, **never** `db.*` (sqlc) or `pgtype.*` types. Those are
  translated at the mapper boundary and must not escape
  `internal/repository`.
- A repository method that needs both `id` and the requesting user's
  `userID` takes them as two explicit `uuid.UUID` params (not a struct)
  when there are only two — reserve `Input` structs for operations with
  optional/growing field sets (create/update/search), not simple
  lookups.

## 5. Interface Design: Prefer Shared Method Signatures Over Duplicated Implementations

Go satisfies interfaces **structurally**: a concrete method satisfies
every interface that declares a method with the *same name and
signature* — there's no explicit `implements` declaration. This has a
direct consequence for how you design a new interface that needs a
capability another interface already exposes:

**Before adding a new interface, check whether an existing method
already has the exact signature you need.** If it does, name your new
interface's method identically, and let the existing concrete
implementation satisfy both interfaces — don't write a second method
with different name but identical logic.
...
the intent ("this one type deliberately satisfies two roles") is
visible at a glance.
...
- `router.go` stays a single flat file until the number of domains makes
  it unwieldy to scan; split into `<domain>_routes.go` only then.
- Small file size is not, by itself, a reason to merge two files that
  serve different concerns (see §2 on mappers) — the split exists to
  separate *reasons to change*, not to hit a line-count target.
...
- **Write comments for WHY, never WHAT.** Well-named identifiers already
  say what the code does. A comment earns its place only when it
  explains a hidden constraint, a subtle invariant, a workaround for a
  specific bug, or behavior that would surprise a reader (this mirrors
  the top-level project guidance and is enforced here specifically for
  `api/`).
- **English only, everywhere, no exceptions** — code, comments, docs,
  example strings. A single stray non-English comment
  (`// Login selalu memulai chain baru.`) made it into `auth_usecase.go`
  before being caught in review; don't let it happen again.
- **Never leave a scaffold/draft comment unresolved in committed code.**
  Phrases like *"if you like"*, *"log if needed, or ignore"*,
  *"assuming this is your util return type"*, or a NOTE addressed to a
  future reader as an instruction rather than documentation are a sign
  the decision was never actually made. Two real instances were found
  and removed from this codebase: a stale NOTE in
  `validate.ErrorResponse` referencing nonexistent `pkg/errs`/`pkg/util`
  paths, and a swallowed-error comment in `config.LoadConfig` that never
  decided whether a malformed `.env` should be visible. **Resolve the
  decision and delete the comment before merging — don't let "TODO for
  you" comments survive review.** If something is genuinely future work,
  it belongs in `TODO.md`, not as an inline comment implying the
  decision is still open in shipped code.
- Don't reference the current task, PR, or issue number in a comment
  ("fix for #123", "added for the X feature") — that context belongs in
  the commit message / PR description and rots as the code evolves.

## 9. Error Handling Standard

- **Reuse an existing `errs.Err*` sentinel before inventing a new one.**
  Check `internal/errs/app_error.go` first.
- **Sentinel identity matters — `errors.Is` is pointer-equality here.**
  `errs.AppError` has no `Is()` or `Unwrap()` method, so
  `errors.Is(err, errs.ErrNotFound)` only succeeds if `err` (or something
  it wraps via `%w`) is *literally* the same `*AppError` pointer as the
  sentinel. This matters because several usecase/handler paths branch on
  it (`auth_usecase.go`'s Login/Refresh/Logout, `AuthHandler.Logout`'s
  idempotency). **Before adding a dynamic/parameterized error constructor**
  (e.g. one that formats a resource name + id into the message), check
  every `errors.Is` call site against the sentinel it's meant to stand
  in for — a dynamically-constructed instance will silently fail all of
  them unless it wires an `Unwrap()` back to the real sentinel. This
  exact mistake (`errs.NewResourceNotFoundError`) was caught as dead code
  before it could be adopted and cause exactly this bug — see PR #2.
- **Document not-found semantics per repository method** when it isn't
  the default. Two valid behaviors exist for a query that matches zero
  rows, and callers need to know which one they're getting:
  - Returns `errs.ErrNotFound` (default for `:one` sqlc queries via
    `errors.Is(err, pgx.ErrNoRows)`).
  - Silent no-op (for `:exec` sqlc queries with no rows-affected count —
    e.g. `SoftDelete`). Document this explicitly in a doc comment, and
    point callers at the `GetByID`-first pattern if they need a real 404.
- **Wrap once per layer crossing**: `fmt.Errorf("<lowercase action
  phrase>: %w", err)` when an error crosses from repository → usecase,
  or usecase → handler-visible. Don't re-wrap an error that's already
...
- **Log unexpected errors exactly once**, in
  ...
  error. `usecase` and `repository` never log; they only return
  (optionally wrapped) errors and let them bubble up.
- **Log every successful mutation, never a successful read**, from the
...
2. Imports, `goimports`-grouped: stdlib, blank line, third-party, blank
   line, internal (`github.com/ngodingvareng/memoria/...`).
3. Package-level `const` (grouped in a `const (...)` block if more than
   one).
4. Compile-time interface assertions (`var _ Interface = (*impl)(nil)`),
   immediately after imports/consts, before the type they assert.
5. Interface declarations (repository interfaces before the usecase
   interface, in `usecase` files).
6. Input/Output structs.
7. The concrete struct + its `New<Type>` constructor.
8. Methods, **in the same order they're declared in the interface** —
   don't let implementation order drift from the interface's declared
   order; it's the first thing a reviewer scans to check completeness.
...
- [ ] No two methods with identical logic implementing different
      interface names for the same capability — rename to share one
      method signature instead (§5).
...
- [ ] Every new mutating handler logs a success line; every new
      not-found-capable repository method documents its zero-match
      semantics in a doc comment (§9).
- [ ] Any new dynamic/parameterized `AppError` constructor has been
      checked against existing `errors.Is` call sites, or deliberately
      wires an `Unwrap()` back to the sentinel it's meant to represent
      (§9).
- [ ] New/changed interfaces have a matching regenerated mock
      (`make mock-generate`) committed in the same PR.
- [ ] File hasn't silently crossed the §6 size thresholds without a
      split plan.
- [ ] `go build ./...`, `go vet ./...`, and `go test -tags=unit ./...`
