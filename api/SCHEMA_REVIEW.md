...
| 11  | Unrestricted `activity_captures.status` transitions           | Data integrity       | Open (product decision)         |
...
**Mitigation in place:** `ConfirmActivityCapture` and `MarkOverdueItemsAsMissed`
...
**Mitigation in place:** `uq_activity_captures_activity_scheduled_at` (a partial
unique index on `(activity_id, scheduled_at) WHERE deleted_at IS NULL`)
rejects the second insert. `CreateScheduledActivityCapture` already uses
...
**Scenario:** `activity_captures.schedule_id` originally only referenced
...
`UNIQUE (activity_id, id)`, and `activity_captures` enforces a composite FK:
...
insert-new-row (instead of `UPDATE` in place), every `activity_capture` that
...
values into `activity_schedule_histories` _before_ the update, in the same
transaction.
...
consider storing a computed `expires_at` directly on each `activity_capture` at
...
**Scenario:** `activity_images.image_path` / `activity_capture_images.image_path`
...
## 11. Unrestricted `activity_captures.status` Transitions
...
`(activity_captures.occurred_at AT TIME ZONE users.timezone)::date`.
