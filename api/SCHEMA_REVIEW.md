...
**Scenario:** A Moment is `due`. The timeout worker evaluates it as
...
**Mitigation in place:** `RecordMoment` and `MarkOverdueMomentsAsMissed`
both guard with `WHERE status = 'due'`, so whichever write lands first
...
**Mitigation in place:** `uq_moments_thread_id_due_at` (a partial
unique index on `(thread_id, due_at) WHERE deleted_at IS NULL`)
rejects the second insert. `CreateCommittedMoment` already uses
`ON CONFLICT ... DO NOTHING` to make that the expected, non-error outcome.

## 3. Concurrent Commitment Edits

**Scenario:** Two requests to edit the same commitment's `cron_expression`
(e.g. a double-submitted form) could race, each archiving a slightly
different "old" snapshot into `commitment_histories`, producing
...
`commitments(id)`, with nothing checking that the commitment's
`thread_id` matched the Moment's own `thread_id`. A bug could link a Moment
to a commitment belonging to an unrelated thread.

**Resolution:** `commitments` now has
`UNIQUE (thread_id, id)`, and `moments` enforces a composite FK:
...
action is what lets only `commitment_id` be nulled out when a commitment is
deleted, instead of also nulling `thread_id` (which is `NOT NULL` and
...
- A Thread has `has_commitment = true` but zero rows in
  `commitments` — auto-generation silently stops working and nothing
  surfaces that fact.
- A Thread is toggled to `has_commitment = false` but old commitment
  rows are never cleaned up, and a buggy generator job that doesn't check
  the flag keeps producing Moments anyway.

**Status:** Not enforced by the schema — purely an application invariant.
`SetThreadHasCommitment`'s query comment flags this, but the app must
explicitly retire/archive commitments whenever the flag flips to `false`, and
the generator job must always join on `threads.has_commitment = true`
(see `ListCommitmentsForGeneration`) rather than iterating
`commitments` alone.
...
insert-new-row (instead of `UPDATE` in place), every `moment` that
referenced the old row via `commitment_id` loses that link (`commitment_id`
becomes `NULL` per the `ON DELETE SET NULL` rule), even though
`commitment_histories` still records the cron change.

**Recommendation:** Always edit via `UPDATE` on the same row
(`UpdateCommitment` in the query set does this), archiving the old
values into `commitment_histories` _before_ the update, in the same
transaction.
...
`due_at + threads.confirmation_timeout_minutes` rather than storing
a fixed deadline per Moment. If a user shortens the timeout (e.g. from 1440 to
60 minutes), every still-`due` Moment — including old ones created under
...
consider storing a computed `expires_at` directly on each `moment` at
creation time, instead of deriving it from the Thread's current setting.
...
**Scenario:** `thread_images.image_path` / `moment_images.image_path`
...
verify flow only deletes the row _after_ using it, two near-simultaneous
...
`kept` (a late confirmation), or a `kept` Moment reverting to
`due`. Whether late confirmation should be allowed at all is a product
...
**Scenario:** The heatmap and other statistics group Moments by "day," but a
day boundary depends on whose timezone you're using. `occurred_at` and
`due_at` are absolute instants (`TIMESTAMPTZ`); without a stored
